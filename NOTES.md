...
```
Game Code:  12 of 498 files complete   1,000 / 2,115,452 bytes   31 / 10,559 fn
All:        1.69% matched              main.dol reproduces byte for byte
```
...
2. **Two near-misses with exhausted searches.** Do not redo these:
...
3. **More units.** `tools/dwarf_targets.py` ranks them. The remaining
   no-data tier includes `FixedAllocator.cpp` (5 fn), `xOGModelRefPtr.cpp`
   (4 fn), `zPathFinderNodeFinder.cpp` (3 fn), `Scaleform.cpp` (5 fn).
