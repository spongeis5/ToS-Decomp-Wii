...
//
// zLedge, transplanted from Sext::zEventSpyAsset::Create, which
...
// virtuals is defined, which is what leaves __vt__zLedge
...
class zLedge : public World::xOGEntity {
public:
    zLedge(World::EntityHandleBase* handle) : World::xOGEntity(handle) {}
    ...
    virtual void Init(Sext::Ledge* asset);

    unsigned char _pad0[0xA8 - 0x4];
};
...
class Ledge {
public:
    static ::zLedge* Create(World::EntityHandleBase* handle,
                            Ledge* asset);
};

}  // namespace Sext

zLedge* Sext::Ledge::Create(World::EntityHandleBase* handle,
                            Ledge* asset) {
    ::zLedge* entity = new (memset(
        Memory::AllocGlobalHeap(sizeof(::zLedge), (Memory::GlobalHeapEnum)0,
                                (eMemMgrTag)16, false),
        0, sizeof(::zLedge))) ::zLedge(handle);
