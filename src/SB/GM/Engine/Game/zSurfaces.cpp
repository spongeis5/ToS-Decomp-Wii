// zSinkingSurface's Create was emitted by tools/transplant.py; the Init below
// was added by hand, so the generated banner is gone and this file
// counts as written. The Create is unchanged.
//
// zSinkingSurface, transplanted from Sext::zEventSpyAsset::Create, which
...
// virtuals is defined, which is what leaves __vt__zSinkingSurface
...
namespace Sext { class zSinkingSurfaceAsset; }
...
class zSinkingSurface : public zInteractionUP {
public:
    zSinkingSurface(World::EntityHandleBase* handle) : zInteractionUP(handle) {}
    ...
    void (*eventFunc)(xBase* from, xBase* to, unsigned int event,
                      Sext::EventAny* any);
    unsigned char _pad2[0x50 - 0x34];
    void* asset;
    unsigned char _pad3[0x88 - 0x54];
};

namespace Sext {

class zSinkingSurfaceAsset {
public:
    static ::zSinkingSurface* Create(World::EntityHandleBase* handle,
                                     zSinkingSurfaceAsset* asset);
};

}  // namespace Sext

zSinkingSurface* Sext::zSinkingSurfaceAsset::Create(World::EntityHandleBase* handle,
                                                    zSinkingSurfaceAsset* asset) {
    ::zSinkingSurface* entity = new (memset(
        Memory::AllocGlobalHeap(sizeof(::zSinkingSurface), (Memory::GlobalHeapEnum)0,
                                (eMemMgrTag)16, false),
        0, sizeof(::zSinkingSurface))) ::zSinkingSurface(handle);
