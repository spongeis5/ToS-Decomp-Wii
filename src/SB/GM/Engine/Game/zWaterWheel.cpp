// zWaterWheel -- the asset's Create, read from the image with
// tools/disasm.py. It is one shape 33 Sext assets in this tree
// share: take sizeof(zWaterWheel) from the global heap (heap 0, tag
...
namespace Sext { class zWaterWheelAsset; }


class zWaterWheel {
public:
    zWaterWheel(World::EntityHandleBase* handle,
                Sext::zWaterWheelAsset* asset);
    ...
    unsigned char _pad1[0xA8 - 0x68];
};


int* zWaterWheel::DriveGetCurMat(int a0) { return &f64; }

namespace Sext {

class zWaterWheelAsset {
public:
    static zWaterWheel* Create(World::EntityHandleBase* handle,
                                 zWaterWheelAsset* asset);
};

}  // namespace Sext

zWaterWheel* Sext::zWaterWheelAsset::Create(World::EntityHandleBase* handle,
                                          zWaterWheelAsset* asset) {
    return new (memset(Memory::AllocGlobalHeap(
                           sizeof(zWaterWheel), (Memory::GlobalHeapEnum)0,
                           (eMemMgrTag)16, false),
                       0, sizeof(zWaterWheel))) zWaterWheel(handle, asset);
}
