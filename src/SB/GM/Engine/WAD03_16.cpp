...
// kept unchanged; the asset Create and zProjectileHavok's destructor
// were added by hand, so the generator's banner is gone -- gen_units.py
...
// zProjectileHavok's destructor is the compiler's own: the null-this
// test, the member at +56 destroyed with the don't-delete flag, then
// the BASE on `this` with the flag CLEAR, then operator delete when the
// caller's flag is positive, and `return this`. r4 = 0 on the second
// call is what says base rather than member.
//
...
// neither the member's real type nor the base's is in the linked image;
// each is spelled as the object the branch reaches.
//
...
class zProjectileHavok : public hkBaseObject {
public:
    virtual void __key();
    ~zProjectileHavok();

    unsigned char _pad0[0x38 - 0x4];
    hkBaseObject f38;
};

zProjectileHavok::~zProjectileHavok() {}
