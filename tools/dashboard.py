...
def main():
    ...
    # GENERATED vs written. A generated source says so in its first line, so
    # this is read from the sources rather than from a list that would rot.
    # 218 of 294 matched functions are generated; reporting one number for
    # both is the flattering figure this project has had to correct before.
    gen_fn = gen_units = 0
    for u in rep["units"]:
        meta = u.get("metadata", {})
        if "game" not in (meta.get("progress_categories") or []):
        ...
        if not p.exists():
            continue
        head = p.read_text(encoding="utf-8", errors="ignore")[:120]
        if "GENERATED by tools/gen_typeids.py" in head:
            gen_fn += u["measures"].get("matched_functions", 0)
            gen_units += 1
    ...
    html = TEMPLATE % {
        ...
        "nmrows": "\n".join(nm_rows),
        "genfn": comma(gen_fn),
        "writtenfn": comma(g["matched_functions"] - gen_fn),
        "genunits": gen_units,
        "blunits": bl_units,
