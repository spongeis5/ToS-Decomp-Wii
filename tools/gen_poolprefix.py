...
def padding_only(args, us, raw, secs, funcs, objs, me_s, me_e):
    """A unit that builds no pool of its own but loads float literals still
    sits in a unity translation unit whose files ahead push its literals
    past 32 KB of .rodata. The TU is the pool whose referrers span the
    unit; the header carries the measured padding and nothing else."""
    tu = None
    for base, size in pools():
        ...
        if lo <= me_s <= hi:
            if tu is not None:
                raise SystemExit("gen_poolprefix: %s lies in two pools' "
                                 "spans, %08X and %08X"
                                 % (args.unit, tu[0], base))
            tu = (base, lo, hi)
    if tu is None:
    ...
    body = [
        ...
        "//",
        "// %s is a fragment of a unity translation unit and builds no"
        % Path(args.unit).name,
...
def main():
    ...
    for base, size in pools():
        ...
        if ref:
            ...
            mine = (base, size)
    if mine is None:
        return padding_only(args, us, raw, secs, funcs, objs, me_s, me_e)
    base, size = mine
    ...
    # `-str reuse` folds two IDENTICAL literals onto one address, which
    # would pull every later string one byte down and bake a constant
    # offset into every instruction that reaches them. A literal
    # repeating one already emitted is therefore written as part of its
    # predecessor with an explicit NUL between: adjacent C literals
    # concatenate, so the bytes are unchanged and there is no second
    # literal left to fold away. CMeshBlobEntity is the unit that has
    # one, and GetPhysicsShape missed by exactly the byte it cost.
    groups = []
    seen = set()
    for off, lit in prefix:
        if lit in seen and groups:
            groups[-1][1].append(lit)
        else:
            seen.add(lit)
            groups.append((off, [lit]))
    ...
    # The array holds LITERALS, and after a fold that is fewer than the
    # number of strings. A header that counts one and shows the other
    # is a header that disagrees with itself.
    if len(groups) != len(prefix):
        body[-1:-1] = [
            "//",
            "// They are emitted as %d literals, not %d: a literal that"
            % (len(groups), len(prefix)),
    ...
    joiner = ' "' + BS + '0" '
    for off, parts in groups:
        lit = joiner.join(c_string(p) for p in parts)
        if len(parts) == 1:
            body.append("    %s,  // +%d" % (lit, off))
        else:
            body.append("    %s,  // +%d, %d literals: the repeat(s)"
                        % (lit, off, len(parts)))
            body.append("        // would fold onto the first copy under -str reuse")
    body += ["};", ""]
    text = NL.join(body)
    if args.check:
        old = out.read_text(encoding="utf-8") if out.exists() else None
