...
SEXT_CREATE_INIT = NL.join([
    ...
    "    %(entity)s* entity = new (memset(",
    "        Memory::AllocGlobalHeap(sizeof(%(entity)s),"
    " (Memory::GlobalHeapEnum)0,",
    "                                (eMemMgrTag)16, false),",
    "        0, sizeof(%(entity)s))) %(entity)s(handle);",
    "",
    "%(initcall)s",
    "",
    "    return entity;",
...
def render(env, target, donor, holes):
    ...
    r = subprocess.run([sys.executable, str(ROOT / "tools/dwarf_lines.py"),
                       ...
                       capture_output=True, text=True)
    m = re.search(r"declared in (\S+)", r.stdout)
    where = m.group(1) if m else "no file the line table names"
    ...
    if d["kind"] == "member":
        ...
        initcall = "    %s(%s, %s);" % (d["init"], arg(p0, "entity"),
                                        arg(p1, "asset"))
    body = tmpl % {
        "entity": d["entity"], "asset": d["asset"], "assetleaf": leaf,
